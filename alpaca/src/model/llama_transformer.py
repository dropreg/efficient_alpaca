...
class LLaMATransformer(nn.Module):
    ...
    def __init__(self, cfg, tgt_dict, embed_tokens, lora_tuning):
        ...
        self.output_projection = Linear(
            self.embed_dim, len(self.tgt_dict), bias=False
        )
...
class LLaMAAttention(nn.Module):
    ...
    def forward(
        ...
        if self.lora_tuning:
            ...
            q = q + self.q_lora(query)
            k = k + self.k_lora(key_value)
            v = v + self.v_lora(key_value)
...
class LLaMAFeedForward(nn.Module):
    ...
    def __init__(self, embed_dim: int, hidden_dim: int):
        ...
        self.w1 = Linear(
            self.embed_dim, self.hidden_dim, bias=False,
        )
        self.w2 = Linear(
            self.hidden_dim, self.embed_dim, bias=False,
        )
        self.w3 = Linear(
            self.embed_dim, self.hidden_dim, bias=False,
        )
